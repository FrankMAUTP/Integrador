...
function addCompetenciaRow(value) {
  ...
  row.innerHTML = `
    <input type="text" class="form-control" placeholder="Nombre de la competencia *" value="${escapeHtml(value)}" oninput="this.value=this.value.replace(/[^a-zA-ZáéíóúüñÁÉÍÓÚÜÑ ]/g,'')" />
...
function validateCompetencias(comps) {
  ...
  for (const c of comps) {
    if (/[^a-zA-ZáéíóúüñÁÉÍÓÚÜÑ ]/.test(c)) { showToast(`La competencia "${c}" solo puede contener letras`, 'error'); return false; }
...
function saveCourse() {
  ...
  if (/[^a-zA-ZáéíóúüñÁÉÍÓÚÜÑ ]/.test(name)) { showToast('El nombre del curso solo puede contener letras', 'error'); return; }
